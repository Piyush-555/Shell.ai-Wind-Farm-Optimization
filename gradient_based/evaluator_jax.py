...
def binWindResourceData(wind_data_file_name):
    ...
    wind_inst_freq = jnp.array(wind_inst_freq)
...
def preProcessing(power_curve):
    ...
    cos_dir = jnp.array(cos_dir)
    sin_dir = jnp.array(sin_dir)
    wind_sped_stacked = jnp.array(wind_sped_stacked)
    C_t = jnp.array(C_t)
    
    return(n_wind_instances, cos_dir, sin_dir, wind_sped_stacked, C_t)


def searchSorted_jax(lookup, sample_array):
    """
    method searchsorted in jax
    """
    lookup_middles = lookup[1:] - jnp.diff(lookup.astype('f'))/2
    idx1 = jnp.searchsorted(lookup_middles, sample_array)
    indices = jnp.arange(lookup.shape[0])[idx1]
    return indices


def soft_argmax(values, axis=1, beta=100):
    assert axis in (0, 1, -1)
    if axis==0:
        values = values.T

    softmax = jax.nn.softmax(beta * values, axis=-1)
    indices = jnp.arange(values.shape[1])
    return jnp.matmul(softmax, indices)


def soft_argmin(values, axis=1, beta=12):
    return soft_argmax(-values, axis, beta)
...
def getAEP(turb_rad, turb_coords, power_curve, wind_inst_freq, 
    ...
    rotate_coords   =  jnp.zeros((n_wind_instances, n_turbs, 2), dtype=jnp.float32)
    # Coordinate Transformation. Rotate coordinates to downwind, crosswind coordinates
    rotate_coords = ops.index_update(rotate_coords, ops.index[:, :, 0],
                    jnp.matmul(cos_dir, jnp.transpose(turb_coords[:,0].reshape(n_turbs,1))) - \
                    jnp.matmul(sin_dir, jnp.transpose(turb_coords[:,1].reshape(n_turbs,1)))
                )
    rotate_coords = ops.index_update(rotate_coords, ops.index[:, :, 1],
                    jnp.matmul(sin_dir, jnp.transpose(turb_coords[:,0].reshape(n_turbs,1))) + \
                    jnp.matmul(cos_dir, jnp.transpose(turb_coords[:,1].reshape(n_turbs,1)))
                )
    
    # x_dist - x dist between turbine pairs wrt downwind/crosswind coordinates)
    # for each wind instance
    x_dist = jnp.zeros((n_wind_instances,n_turbs,n_turbs), dtype=jnp.float32)
    for i in range(n_wind_instances):
        tmp = rotate_coords[i,:,0].repeat(n_turbs).reshape(n_turbs, n_turbs)
        x_dist = ops.index_update(x_dist, i, tmp - tmp.transpose())


    # y_dist - y dist between turbine pairs wrt downwind/crosswind coordinates)
    # for each wind instance    
    y_dist = jnp.zeros((n_wind_instances,n_turbs,n_turbs), dtype=jnp.float32)
    for i in range(n_wind_instances):
        tmp = rotate_coords[i,:,1].repeat(n_turbs).reshape(n_turbs, n_turbs)
        y_dist = ops.index_update(y_dist, i, tmp - tmp.transpose())
    y_dist = jnp.abs(y_dist) 
    ...
    sped_deficit = (1-jnp.sqrt(1-C_t + 1e-8))*((turb_rad/(turb_rad + 0.05*x_dist))**2) 
    sped_deficit = ops.index_update(sped_deficit, ops.index[((x_dist <= 0) | ((x_dist > 0) & (y_dist > (turb_rad + 0.05*x_dist))))], 0.0)

    # Calculate Total speed deficit from all upstream turbs, using sqrt of sum of sqrs
    sped_deficit_eff = jnp.sqrt(jnp.sum(jnp.square(sped_deficit), axis = 2) + 1e-8)
    ...
    # indices = searchSorted_jax(power_curve[:,0], wind_sped_eff.ravel())
    pre_indices = jnp.abs(jnp.expand_dims(power_curve[:,0], 1) - jnp.expand_dims(wind_sped_eff.ravel(), 0))
    indices = soft_argmin(pre_indices, 0, beta=15)
    indices_hard = pre_indices.argmin(axis=0)
    indices_onehot = jnp.eye(501)[indices_hard]
    power = jax.lax.stop_gradient((indices_onehot @ power_curve[:, 2]) - indices) + indices
    ...
    # year_hours = 8760.0
    AEP = 8760.0 * jnp.sum(power * wind_inst_freq)
...
if __name__ == "__main__":
    ...
    # submission_path = input('Enter submission file path: ')
    ...
    turb_coords   =  getTurbLoc("Submissions/original.csv")
